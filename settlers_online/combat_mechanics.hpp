...
namespace ropufu
{
    namespace settlers_online
    {
        ...
        private:
            ...
            bool m_is_uniform_splash; // Indicates that there is no effective damage reduction that can prevent splash optimization.
        }
        ...
        private:
            ...
            std::vector<attack_group_cache> m_attacker_caches = { }; // Additional parameters determining how each group will attack the enemy.
         ...
         *  @param other The unconditioned(!) army to prepare to fight against.
         */
        conditioned_army::conditioned_army(const army& a, const army& other)
            : m_army(a), m_counts(a.counts_by_type()), m_attack_orders()
        {
            ...
            for (unit_group& g : this->m_army.groups())
            {
                ...
                if (other.do_intercept())
                {
                    ...
                    low_damage = fraction_ceiling(intercept_damage_percent * low_damage, 100);
                    high_damage = fraction_ceiling(intercept_damage_percent * high_damage, 100);
                }
                // Trait: dazzle.
            ...
            // Now that the army has been conditioned, determine.
            this->m_attacker_caches.reserve(this->m_army.count_groups());
            for (unit_group& g : this->m_army.groups())
            {
                ...
                bool has_effective_tower_bonus = (!do_ignore_tower_bonus && other.has_tower_bonus());
                double damage_factor_normal = (1.0 - other.direct_damage_reduction());
                double damage_factor_in_tower = (1.0 - other.direct_damage_reduction()) * (1.0 - other.tower_damage_reduction());
                ...
                bool is_uniform_splash = (!has_effective_tower_bonus && attacker_t.splash_chance() == 1.0);

                // Figure out which ordering to use.
                bool do_attack_weakest_target = t.has(special_ability::attack_weakest_target);
                const aftermath::algebra::permutation& defender_ordering = do_attack_weakest_target ? other.order_by_hp() : other.order_by_id();
                this->m_attacker_caches.push_back(defender_ordering);
            }
            this->m_attacker_caches.shrink_to_fit();
        }
        ...
        public:
            ...
            combat_result execute(attack_sequence<t_left_sequence_type>& left_sequencer, attack_sequence<t_right_sequence_type>& right_sequencer)
            {
                ...
                {
                    if (this->m_do_log) std::cout << "Begin round " << count_rounds << std::endl;
                    for (std::size_t k = 0; k < count_phases; k++)
                    {
                        battle_phase phase = static_cast<battle_phase>(k);
                        if (this->m_do_log) std::cout << "Begin phase " << std::to_string(phase) << std::endl;

                        for (std::size_t i : this->left_group_indices[phase]) combat_mechanics::hit(this->m_right, this->m_left[i], left_frenzy_factor, left_sequencer, this->m_do_log);
                        for (std::size_t i : this->right_group_indices[phase]) combat_mechanics::hit(this->m_left, this->m_right[i], right_frenzy_factor, right_sequencer, this->m_do_log);
        ...
        private:
            ...
            template <typename t_sequence_type>
            static void hit(army& defender, const unit_group& attacking_group, double frenzy_factor, attack_sequence<t_sequence_type>& sequencer, bool do_log)
            {
                ...
                if (!has_effective_tower_bonus && attacker_t.splash_chance() == 1.0)
                {
                    ...
                    combat_mechanics::uniform_splash(total_reduced_damage, defender, defender_ordering, do_log);
                    return;
                }
                ...
                for (std::size_t j : defender_ordering)
                {
                    ...
                    overshoot_damage = combat_mechanics::splash(overshoot_damage, damage_factor, defending_group, do_log);
                    ...
                    if (is_one_to_one) attacking_unit_index = combat_mechanics::one_to_one(defending_group, attacking_group, attacking_unit_index, do_log);
                    else attacking_unit_index = combat_mechanics::hit(overshoot_damage, defending_group, attacking_group, attacking_unit_index, damage_factor, sequencer, do_log);
        ...
        std::size_t combat_mechanics::splash(std::size_t pure_damage, double damage_factor, unit_group& defending_group, bool do_log)
        {
            ...
            if (do_log) std::cout << '\t' << "splash killing " << (defending_units_remaining - defending_group.count()) << " " << defending_group.type().names().front() << std::endl;
            return pure_damage;
        }

        void combat_mechanics::uniform_splash(std::size_t reduced_damage, army& defender, const aftermath::algebra::permutation& defender_ordering, bool do_log)
        {
            std::vector<unit_group>& defender_groups = defender.groups();
            for (std::size_t j : defender_ordering)
            {
                ...
                if (do_log)
                {
                    std::cout <<
                        defending_units_remaining << " " <<
                        defending_group.type().names().front() << " killing ";
                }
                ...
                if (total_hit_points >= reduced_damage) // All the splash damage has been accounted for.
                {
                    ...
                    if (do_log) std::cout << (defending_units_remaining - defending_group.count()) << ".";
                    return;
                }
                defending_group.kill_all();
                if (do_log) std::cout << defending_units_remaining << ".";
                reduced_damage -= total_hit_points;
            }
        }

        std::size_t combat_mechanics::one_to_one(unit_group& defending_group, const unit_group& attacking_group, std::size_t attacking_unit_index, bool do_log)
        {
