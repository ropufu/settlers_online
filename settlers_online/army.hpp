...
namespace ropufu
{
    namespace settlers_online
    {
        ...
        private:
			template <typename t>
			using cat_array = std::array<t, settlers_online::category_capacity>;
			...
			cat_array<std::size_t> m_min_damage_bonus;  // Increases (additive) the \c min_damage by category.
            cat_array<std::size_t> m_max_damage_bonus;  // Increases (additive) the \c max_damage by category.
        ...
        public:
            ...
            /** Increases (additive) the \c min_damage by category. */
            const cat_array<std::size_t>& min_damage_bonus() const noexcept { return this->m_min_damage_bonus; }
            /** Increases (additive) the \c max_damage by category. */
            const cat_array<std::size_t>& max_damage_bonus() const noexcept { return this->m_max_damage_bonus; }

            /** Increases (additive) the \c accuracy for a specific category. */
            double accuracy_bonus_for(unit_category category) const noexcept { return this->m_accuracy_bonus[static_cast<std::int32_t>(category)]; }
            /** Increases (additive) the \c accuracy for a specific category. */
            void set_accuracy_bonus_for(unit_category category, double value) noexcept { this->m_accuracy_bonus[static_cast<std::int32_t>(category)] = value; }

            /** Increases (additive) the \c splash_chance for a specific category. */
            double splash_bonus_for(unit_category category) const noexcept { return this->m_splash_bonus[static_cast<std::int32_t>(category)]; }
            /** Increases (additive) the \c splash_chance for a specific category. */
            void set_splash_bonus_for(unit_category category, double value) noexcept { this->m_splash_bonus[static_cast<std::int32_t>(category)] = value; }

            /** Increases (additive) the \c min_damage for a specific category. */
            std::size_t min_damage_bonus_for(unit_category category) const noexcept { return this->m_min_damage_bonus[static_cast<std::int32_t>(category)]; }
            /** Increases (additive) the \c min_damage for a specific category. */
            void set_min_damage_bonus_for(unit_category category, std::size_t value) noexcept { this->m_min_damage_bonus[static_cast<std::int32_t>(category)] = value; }

            /** Increases (additive) the \c max_damage for a specific category. */
            std::size_t max_damage_bonus_for(unit_category category) const noexcept { return this->m_max_damage_bonus[static_cast<std::int32_t>(category)]; }
            /** Increases (additive) the \c max_damage for a specific category. */
            void set_max_damage_bonus_for(unit_category category, std::size_t value) noexcept { this->m_max_damage_bonus[static_cast<std::int32_t>(category)] = value; }
            ...
            /** Ordering by unit initiative. */
            const aftermath::algebra::permutation& order_by_initiative() const noexcept { return this->m_order_by_initiative; }
            ...
            /** Access groups ordered by initiative. */
            const unit_group& by_initiative(std::size_t index) const noexcept
            {
                return this->m_groups[this->m_order_by_initiative[index]];
            }
            ...
            bool has_tower_bonus() const noexcept
            {
                ...
                for (const unit_group& g : this->m_groups) if (g.type().has(special_abilities::tower_bonus)) return true;
                return false;
            }
            ...
            bool operator ==(const army& other) const noexcept
            {
				...
				return
                    ...
                    this->m_min_damage_bonus == other.m_min_damage_bonus &&
					this->m_max_damage_bonus == other.m_max_damage_bonus;
            }
            ...
            m_accuracy_bonus(), m_splash_bonus(), m_min_damage_bonus(), m_max_damage_bonus()
        {
            ...
            this->m_order_by_initiative.order_by(
                this->m_groups, [](const unit_group& x) { return x.type(); }, unit_type::compare_by_initiative);

			// Initialize category arrays.
			this->m_accuracy_bonus.fill(0);
            this->m_splash_bonus.fill(0);
			this->m_min_damage_bonus.fill(0);
            this->m_max_damage_bonus.fill(0);
...
namespace std
{
    ...
    struct hash<ropufu::settlers_online::army>
    {
        ...
        result_type operator()(const argument_type& x) const
        {
			...
			for (std::size_t y : x.min_damage_bonus()) h ^= size_hash(y);
            for (std::size_t y : x.max_damage_bonus()) h ^= size_hash(y);
